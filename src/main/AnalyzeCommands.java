        ...
        for (CommandIn command: commands) {
            ...
            if (command.getCommand().contains("search")) {
                ...
                for (Playlist iter: currentUser.getPlaylists()) {
                    if (iter.getDetails().getName().equals(command.getPlaylistName())) {
                ...
                if (!isCreated) {
                    ...
                    currentUser.getPlaylists().add(new Playlist(command));
            ...
            */
            for (UserInfo updateTimer: users) {
                if (updateTimer.getPlayer() != null) {
                    updateTimer.updateRemainedTime(command);
                }
            }
        }
        return result;
