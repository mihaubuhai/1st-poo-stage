...
public class Prev extends NP {
    public Prev(final UserInfo user) {
        setCurrentUser(user);
    ...
    public ResultOut prevFunc(final CommandIn command, final LibraryInput library) {
        ...
        if (!checkValidity(command)) {
            ...
            if (playerType == 1) {
                ...
                SongInput currSong = getPlaylist().getDetails().getSongs().get(currSongIdx);
                ...
                */
                if (!currentPlayer.getStats().getShuffle()) {
                    ...
                    int shuffleCurrIdx = getPlaylist().getShuffleIndices().indexOf(currSongIdx);
    ...
    private void checkTime(final SongInput currSong, final int currIdx,
        ...
        if (initDuration - remainedTime >= 1 || currIdx == 0) {
            ...
            SongInput prevSong = getPlaylist().getDetails().getSongs().get(prevIdx);
    ...
    private int findPrevIdxShuffle(final int idx) {
        ...
        ArrayList<Integer> shuffles = getPlaylist().getShuffleIndices();
