...
public class Next extends NP {
    public Next(final UserInfo user) {
        setCurrentUser(user);
    ...
    public ResultOut nextFunc(final CommandIn command, final LibraryInput library) {
        ...
        if (!checkValidity(command)) {
            ...
            if (playerType == 1) {
                ...
                int nextIdx = playlist.findIdxSong(currentUser);
                if (repeatMode.contains("no")) {
                    ...
                    if (nextIdx < 0) {
                        ...
                        SongInput nextSong = getPlaylist().getDetails().getSongs().get(nextIdx);
                    ...
                    SongInput currSong = getPlaylist().getDetails().getSongs().get(idx);
